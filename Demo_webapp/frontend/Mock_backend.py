...
#   - job_uuid: generated by the frontend (03_Data_prep.py), sent in on job
#     creation, and used from then on as the identifier for every API call
#     (status polling, results, notifications). It's what the wire protocol
#     is keyed on.
...
# API call uses.
RESULT_BASE_URL = "http://localhost:8501/Dashboard"
...
def submit_analysis(payload: AnalyzePayload, authorization: Optional[str] = Header(None)):
    ...
    # Register the new job in our mock database, keyed by job_uuid (the
    # identifier every subsequent API call will use). job_id is a separate,
    # backend-generated internal identifier (DB primary key stand-in) --
    # returned once here so the frontend can use it for its own project
    # tracking, but it's never used to address an API call after this.
    ...
    return {"job_id": job_id, "job_uuid": payload.job_uuid, "message": "Job started"}
